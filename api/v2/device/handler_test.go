...
func TestListDevices_Empty(t *testing.T) {
	...
	if len(resp200.Devices) != 0 {
		t.Errorf("Expected empty list, got %d devices", len(resp200.Devices))
	}
	if resp200.Total != 0 {
		t.Errorf("Expected total 0, got %d", resp200.Total)
	}
}
...
func TestListDevices_WithVoucher(t *testing.T) {
	...
	if len(resp200.Devices) != 1 {
		t.Fatalf("Expected 1 device, got %d", len(resp200.Devices))
	}
	if resp200.Devices[0].Guid != guidHex {
		t.Errorf("Expected GUID %s, got %s", guidHex, resp200.Devices[0].Guid)
	}
	if resp200.Devices[0].To2Completed {
...
func TestListDevices_InvalidGUIDFilter(t *testing.T) {
	...
	if _, ok := response.(ListDevices400JSONResponse); !ok {
...
func TestListDevices_ValidGUIDFilterNoMatch(t *testing.T) {
	...
	if len(resp200.Devices) != 0 {
		t.Errorf("Expected 0 devices, got %d", len(resp200.Devices))
	}
}
