...
var (
	newTO0Client = func(vouchers fdo.OwnerVoucherPersistentState, keys fdo.OwnerKeyPersistentState) to0Client {
		return &fdo.TO0Client{Vouchers: vouchers, OwnerKeys: keys, TTL: 300}
	}
	makeTransport  = tls.TlsTransport
	fetchOwnerInfo = db.FetchOwnerInfo
)

func RegisterRvBlob(rvInfo [][]protocol.RvInstruction, to0Guid string, voucherState fdo.OwnerVoucherPersistentState, keyState fdo.OwnerKeyPersistentState, insecureTLS bool) (uint32, error) { // Parse to0-guid flag
